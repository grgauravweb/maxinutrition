...
$.widget('mage.quickSearch',{options:{autocomplete:'off',minSearchLength:2,responseFieldElements:'ul li',selectClass:'selected',template:'<li class="search-main__suggestions__product-item col-xs-6 col-md-4 col-lg-3 util-flex-fill" '+'id="qs-option-<%- data.index %>" role="option">'+'<div class="product-item">'+'<a href="<%- data.url %>" data-redirect="<%- data.url %>" '+'data-redirect-target="_blank" class="product-item__image">'+'<img src="<%- data.image %>">'+'</a>'+'<div class="product-item__description">'+'<a href="<%- data.url %>" data-redirect="<%- data.url %>" data-redirect-target="_blank"'+'class="product-item__name" >'+'<h5><%- data.title %></h5>'+'</a>'+'</div>'+'<div class="product-item__label" style="background-color: '+'<%- data.label_bgcolor %>'+' ; color: '+'<%- data.label_txtcolor %>'+'">'+'<%- data.label_text %>'+'</div>'+'<div class="product-item__additional-info">'+'<span class="product-item__flavor">'+'<%- data.flavour %></span>'+'</div>'+'<div class="product-item-inner">'+'<div class="product-item__actions">'+'<div class="product-item__price">'+'<%= data.price %>'+'</div>'+'</div>'+'</div>'+'</div>'+'</li>',submitBtn:'button[type="submit"]',searchLabel:'[data-role=minisearch-label]',isExpandable:null},_create:function(){this.responseList={indexList:null,selected:null};this.autoComplete=$(this.options.destinationSelector);this.searchForm=$(this.options.formSelector);this.submitBtn=this.searchForm.find(this.options.submitBtn)[0];this.searchLabel=$(this.options.searchLabel);this.isExpandable=this.options.isExpandable;_.bindAll(this,'_onKeyDown','_onPropertyChange','_onSubmit');this.element.attr('autocomplete',this.options.autocomplete);mediaCheck({media:'(max-width: 768px)',entry:function(){this.isExpandable=true;}.bind(this),exit:function(){this.isExpandable=false;this.element.removeAttr('aria-expanded');}.bind(this)});this.searchLabel.on('click',function(e){if(this.isExpandable&&this.isActive()){e.preventDefault();}}.bind(this));this.element.on('blur',$.proxy(function(){if(!this.searchLabel.hasClass('active')){return;}
