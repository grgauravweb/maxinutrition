...
$('body').removeClass('safari-context');}};var addClassToBssPopup=function(){$('.bss_popup').addClass('opened');};var removeClassFromBssPopup=function(){$('.bss_popup').removeClass('opened');};var updatePopupDisplayed=function(popupId){$.ajax({url:config.updateUrl,type:'POST',data:{popupId:popupId}});};var addNewClassToPopup=function($popupObject){if(iosCheck()===1||iosCheck()===2){var newClass="safari-context-wrap";$popupObject.st.mainClass=$popupObject.st.mainClass+' '+newClass;}};var removeClassFromPopup=function($popupObject){if(iosCheck()===1||iosCheck()===2){var newClass="safari-context-wrap";$($popupObject.st.mainClass).removeClass(newClass);}};$(document).ready(function(){var autoClose=config.hideAfter;var timeDelay=(config.effectDisplay)?500:0;var displayed=false;var allowDisplay=config.popupIsAllowedDisplay;function displayPopup(allowDisplay,autoClose,timeDelay){$.ajax({url:config.checkTimeUrl,type:'POST',data:{popupId:config.popupId,isPreview:config.preview?1:0},success:function(result){if(result.res===true){addClassToBody();$(".popup_wrapper").css({"display":"block"});if(typeTemplatePopup==TYPE_TEMPLATE_AGE_VERIFICATION){$.magnificPopup.open({items:{src:'.popup_wrapper',},callbacks:{beforeOpen:function(){addNewClassToPopup(this);if($('body').width()<=1444){addClassToBssPopup();}
mfgPop=this;},close:function(){removeClassFromBody();removeClassFromPopup(this);removeClassFromBssPopup();}},removalDelay:timeDelay,mainClass:config.animation,alignTop:config.flagTop,closeOnBgClick:false,enableEscapeKey:false,showCloseBtn:false});}else{$.magnificPopup.open({items:{src:'.popup_wrapper',},callbacks:{beforeOpen:function(){addNewClassToPopup(this);if($('body').width()<=1444){addClassToBssPopup();}
mfgPop=this;},close:function(){removeClassFromBody();removeClassFromPopup(this);removeClassFromBssPopup();}},removalDelay:timeDelay,mainClass:config.animation,alignTop:config.flagTop,});}
